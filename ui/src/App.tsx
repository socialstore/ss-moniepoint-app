					...
					<Card>
						<CardContent className="flex flex-col items-center gap-1 py-14 text-center">
							...
							<div className="text-sm text-muted-foreground">
								Open this app from your Sentralbee dashboard.
							</div>
						</CardContent>
					</Card>
				) : (
...
function StatusPill({
	...
	return (
		<Badge
			...
			className={cn('gap-1 font-normal', !ok && 'text-muted-foreground')}
		>
...
function Connect() {
	...
	async function save() {
		...
		try {
			...
			setMsg(
				w?.ok
					? `Connected · webhook subscription created (${w.subscriptionId || 'ok'})`
					: `Saved · webhook: ${w?.error ?? 'pending'}`,
			);
			load();
		} catch (e) {
	...
	return (
		<Card>
			<CardHeader>
				...
				<CardDescription>
					Link your Moniepoint account so transfers reconcile to
			...
			<CardContent className="space-y-4">
				<Field
					label="Moniepoint business ID (Merchant ID)"
					v={f.businessId}
					on={(v) => setF({ ...f, businessId: v })}
				/>
				<Field
					label="Moniepoint API token"
					v={f.moniepointApiToken}
					on={(v) => setF({ ...f, moniepointApiToken: v })}
					secret
				/>
				<p className="text-xs leading-relaxed text-muted-foreground">
					Your Sentralbee API access was provisioned automatically on
					install — the app also creates the Moniepoint webhook
					subscription for you, so there are no keys or secrets to
					copy.
				</p>
			</CardContent>
...
function Terminals() {
	...
	async function add() {
		if (!f.terminalSerial || !f.nuban)
			return setMsg('Serial + NUBAN required');
		setBusy(true);
		setMsg('');
		try {
	...
	return (
		<Card>
			<CardHeader>
				...
				<CardDescription>
					POS terminals whose transfers this app reconciles.
				</CardDescription>
			</CardHeader>
			<CardContent className="space-y-4">
				...
				<div className="grid grid-cols-1 gap-2 sm:grid-cols-[1fr_1fr_1fr_auto]">
					<Input
						placeholder="Serial"
						value={f.terminalSerial}
						onChange={(e) =>
							setF({ ...f, terminalSerial: e.target.value })
						}
					/>
					<Input
						placeholder="NUBAN"
						value={f.nuban}
						onChange={(e) => setF({ ...f, nuban: e.target.value })}
					/>
					<Input
						placeholder="Account name"
						value={f.accountName}
						onChange={(e) =>
							setF({ ...f, accountName: e.target.value })
						}
					/>
					<Button
	...
	return (
		<Card>
			<CardHeader>
				...
				<CardDescription>
					Unmatched transfers awaiting reconciliation. Bind one to an
					order to mark it paid.
				</CardDescription>
			</CardHeader>
			<CardContent>
				<div className="overflow-hidden rounded-lg border">
				...
				{msg ? (
					<p className="pt-3 text-xs text-muted-foreground">{msg}</p>
				) : null}
			</CardContent>
		</Card>
	);
